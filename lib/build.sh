...
astyle  --options=$arduino/lib/formatter.conf ../proc_wifi/*.h ../proc_wifi/*.ino
rm -f ../proc_wifi/*.orig
if ! [ -x ./uncrc32 ] ; then
...
fqbn="esp8266:esp8266:generic:xtal=80,vt=flash,exception=disabled,stacksmash=disabled,ssl=all,mmu=4816,non32xfer=fast,CrystalFreq=26,FlashFreq=80,FlashMode=qio,eesz=4M3M,led=2,sdk=nonosdk305,ip=hb2f$debug,wipe=none,baud=921600"
...
rm -f /tmp/${me}_build/proc_wifi.ino.bin
$arduino/arduino-builder \
-compile \
-logger=machine \
-hardware $arduino/hardware \
...
-prefs=build.warn_data_percentage=75 \
-verbose \
./proc_wifi/proc_wifi.ino |tee /tmp/${me}_info.log

if [ -e /tmp/${me}_build/proc_wifi.ino.bin ] ; then
  ...
  cp -a /tmp/${me}_build/proc_wifi.ino.bin lib/proc_wifi.bin
